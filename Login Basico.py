...
def NuevoUsuario():
    ...
    while username.isspace() == True: #Este programa tiene como agregado el uso de isspace, ya que si un usuario utilizara Espacios unicamente le permitiria crear un usuario en blanco
        print("Su usuario no puede estar vacio ")
        username = str(input("A continuacion ingrese un nombre de usuario: "))
    else: pass
    password = str(input("A continuacion ingrese la contraseña para este usuario: "))
    while password.isspace() == True:
...
def login():
    while True:
            ...
            while nombredeusuario.isspace() == True:
